...
model = AR2Model_single(n, phi1=1.347,phi2=-0.36,noise=0.001)
...
model1 = SimpleModel(n, alpha = 0.95, q=10)
meas_idx = { "Pk": PMeasIdx, "Qk":  QMeasIdx, "Vm": VMeasIdx, "Va": VMeasIdx}
simdata1 = network.simulate_data(S, gen=LoadP/(baseMVA*1000), PVdata=PVdata[:,14]/1000,PV_idx=6, verbose = 0) #
pseudo_meas1=network.create_pseudomeas(simdata1,meas_idx,PVdata=PVdata[:,14]/1000,PV_idx=6)
meas = { "Pk": S[:nNodes,:][PMeasIdx,:], "Qk": S[nNodes:,:][QMeasIdx,:],"V": V, "Vm": V[:nNodes,:][VMeasIdx,:], "Va": V[nNodes:,:][VMeasIdx,:]}
meas_unc = { "Vm": 1e-2*np.ones(nVMeas), "Va": 1e-2*np.ones(nVMeas) }
Shat1, Vhat1, uShat1, DeltaS1, uDeltaS1 = IteratedExtendedKalman(topology, meas, meas_unc, meas_idx, pseudo_meas1, model1,Vhat0,Vs,Y=Y)
...
for i in range(nNodes):
	...
	plot(t, -1000*Shat[i,:],'D-m',linewidth=2.5,label="AR model")
	plot(t, -1000*Shat1[i,:],'v-y',alpha=0.5,linewidth=2.5,label="simple model")
